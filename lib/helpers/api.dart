...
class KnockoutAPI {
  ...
  Future<List<Subforum>> getSubforums() async {
    final response = await http.get(_baseurl + 'subforum');

    return parseSubforums(response.body);
  }

  Future<SubforumDetails> getSubforumDetails(int id) async {
    final response = await http.get(_baseurl + 'subforum/' + id.toString());
  ...
  Future<Thread> getThread(int id, {int page: 1}) async {
    final response = await http
        .get(_baseurl + 'thread/' + id.toString() + '/' + page.toString());
  ...
  Future<Thread> authCheck() async {
    ...
    final response = await http.get(_baseurl + 'user/authCheck',
  ...
  Future<void> getAlerts() async {
    final response = await http.get(_baseurl + 'alert/list', headers: {});

    print(response.body);
  }

  Future<void> getEvetns() async {
    final response = await http.get(_baseurl + 'events', headers: {});
