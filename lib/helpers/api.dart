...
const baseurlSite = !isDev ? "https://knockout.chat/" : "https://forums.stylepunch.club/";
const baseurl = !isDev ? "https://api.knockout.chat/" : "https://forums.stylepunch.club:3000/";

List<Subforum> parseSubforums(String responseBody) {
  final parsed = json.decode(responseBody)['list'].cast<Map<String, dynamic>>();
...
Future<List<Subforum>> getSubforums () async {
  final response =
        await http.get(baseurl + 'subforum');
...
Future<SubforumDetails> getSubforumDetails (int id) async {
  final response =
        await http.get(baseurl + 'subforum/' + id.toString());
...
Future<Thread> getThread (int id, {int page: 1}) async {
  final response =
        await http.get(baseurl + 'thread/' + id.toString() + '/' + page.toString());
...
Future<Thread> authCheck () async {
  ...
  final response =
        await http.get(baseurl + 'user/authCheck', headers: {
